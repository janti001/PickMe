...
def filter_objects(
    input_dir: str, filter_choice=None, output_dir=None, non_interactive=False
):
    ...
    # Get all objects
    files = utils.choose_tomograms(
        segmentation_directory=input_dir, non_interactive=non_interactive
    )
    ...
    with tqdm(
        total=len(files), desc="Running Extraction", unit="Tomogram", leave=True
    ) as pbar:
        for file in files:
            try:
                ...
                pbar.set_postfix_str(
                    f"Processing {mgraph}... | shape (zyx)={shape_zyx}"
                )
                ...
                objects_filtered = filter.knee_detection(
                    objects_dictionary=objects_dict,
                ...
                out_path = (
                    f"{os.path.join(output_directory, tomo_name)}_filtered.mrc.gz"
                )
                with mrcfile.new(
                    name=out_path, compression="gzip", overwrite=True
                ) as mrc:
...
def choose_object(
    ...
    if non_interactive:
        ...
        while ask_user.lower() not in ["y", "yes", "n", "no"]:
            ...
            ask_user = input(
                "Which objects of interest would you like to select from the filtered set for processing?"
            )
    ...
    if os.path.isfile(input_dir):
        ...
        tomogram_list = glob.glob(
            os.path.join(input_dir, "*.mrc")
        )  # this assumes that the tomograms are in mrc format - we can change this to be more flexible if needed
    outputs_root = utils.get_output_root(output_dir)
    if segmentation_dir is None and input_job is None:
        extract_jobs = sorted(
            [
                job
                for job in (outputs_root / "filter").glob("job[0-9][0-9][0-9]")
                if job.is_dir()
            ]
        )
        if extract_jobs:
    ...
    valid_id = [
        seg.split("/")[-1].split("_")[1] for seg in filtered_seg_list
    ]  # we only want the tomograms that are in extract job
    for tomogram in tomogram_list:
        ...
        if tomo_id in valid_id:
            ...
            data_dict[tomo_id].update(
                {"segmentation": seg for seg in filtered_seg_list if tomo_id in seg}
            )
    ...
    if ask_user == True:
        ...
        def _analysed_tomo_id() -> str | None:
            ...
            if layer is not None and "_segmentation" in getattr(layer, "name", ""):
                return layer.name.replace(
                    "_segmentation", ""
                )  # this just ensures that we have the tomogram id - TS_xyxy
            return None
        ...
        _, plugin_widget = napari_compat.add_regionprops_widget(viewer)

        _connected_table = (
            None  # hold a reference so we can reconnect on subsequent runs
        )
        ...
        def _on_analysis_finished():
            ...
            if table is None:
                print(
                    "[PickMe] Could not find the regionprops table — click Analyze first."
                )
                # Debug helper: print what dock widgets exist
                print(
                    f"[PickMe] Current dock widgets: {napari_compat.dock_widget_names(viewer)}"
                )
                return
        ...
        def _on_selection_changed(*_args):
            ...
            for row in {index.row() for index in indexes}:
                label = napari_compat.read_label_cell(
                    _connected_table, row, _label_column
                )
        ...
        final_selection = {
            tomo: labels for tomo, labels in selections.items() if labels
        }
        print("\n=== Final selections ===")
        ...
        with tqdm(
            total=len(final_selection), desc="Writing", unit="Tomogram", leave=True
        ) as pbar:
            for tomo_id, chosen_labels in final_selection.items():
                try:
                    ...
                    chosen_coords = object_coords_by_label(
                        segmentation, labels=chosen_labels
                    )
                    segmentation = None
                    ...
                    pbar.set_postfix_str(
                        f"Processing {tomo_id} | shape (zyx)={shape_zyx}"
                    )
                    ...
                    if write_selections == False:
                        ...
                        out_path = os.path.join(
                            output_directory, f"TS_{tomo_id}_filtered_chosen.mrc.gz"
                        )
                        with mrcfile.new(
                            out_path, compression="gzip", overwrite=True
                        ) as new_file:
                        ...
                        out_dir = os.path.join(
                            output_directory, f"TS_{tomo_id}_membranes"
                        )
                        ...
                        for label, coords in chosen_coords.items():
                            ...
                            out_path = os.path.join(
                                out_dir, f"TS_{tomo_id}_obj{label}.mrc"
                            )  # could change this to mrc.gz -> for the purpsoe of doing membrain, will leave it as mrc - will change to give user an option
...
def particle_extract(
    sample_rate: int, cmm: bool, input_dir=None, input_job=None, output_dir=None
):
    ...
    star_dict = {
        ...
        "rlnAnglePsi": [],
        "rlnMicrographName": [],
    }  # This is .tomostar files -> TS_1234.tomostar

    total_star_df = pd.DataFrame.from_dict(star_dict)
    tomogram_star_df = total_star_df.copy()

    # set the output directory
    output_directory = utils.check_make_dir(
        job_name="particle_extraction", directory=output_dir
    )
    ...
    choose_jobs = sorted(
        [
            job
            for job in (outputs_root / "choose").glob("**/job[0-9][0-9][0-9]")
            if job.is_dir()
        ]
    )
    # ---- Getting files
    if isinstance(input_job, (str, int)):
        ...
        input_dir is None and choose_jobs
    ):  # choose obs has to return something - i.e., the choose job has to be run at least once prior if no input directory is provided
        # retrieve the files from the output/choose directory - latest job
        files = list(
            choose_jobs[-1].glob("*chosen*")
        )  # we use glob method with Posix Path as it is a Path object, not a string
    elif input_dir is None and not choose_jobs:
        raise RuntimeError(
            "choose_object job must be run if you are to provide no input directory"
        )
    # possibility of having input dir
    else:
        print("WARNING: files must be in mrc, mrc.gz, mrc.bz2")
        files = (
            glob.glob(f"{input_dir}/*mrc")
            + glob.glob(f"{input_dir}/*mrc.gz")
            + glob.glob(f"{input_dir}/*mrc.bz2")
        )
    ...
    with tqdm(
        total=len(files),
        desc="Extracting particles",
        unit="file",
        dynamic_ncols=True,
        bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [Elapsed(s):{elapsed}<>Remaining(s):{remaining}, {rate_fmt}] {postfix}",
    ) as pbar:
        try:
            for file in files:
                ...
                tomo_name = utils.get_mgraph(
                    file, caller="particle_extract"
                )  # this is .tomostar file
                ...
                for object in objects_dict.values():
                    ...
                    smooth_object = gaussian_filter(
                        object_array.astype(float), sigma=3.0
                    )
                    ...
                    particle_dict = sampling.non_random_membrane_sampling(
                        coords=verts, normal_vectors=normals, grid_sampling=sample_rate
                    )
                    # -- calculate euler angles and other data needed for star file
                    data_entries = angles.euler_star(
                        centre_of_mass=object.centroid,
                        particles=particle_dict,
                        label=int(object.label),
                        micrograph=tomo_name,
                        tomo_dimensions=shape_zyx,
                        psize=pixel_size,
                    )
                    per_tomo_data = pd.concat(
                        [per_tomo_data, pd.DataFrame.from_dict(data_entries)],
                        ignore_index=True,
                    )
                ...
                plotting.plot_angles(
                    star_data=tomogram_star_df,
                ...
                starfile.write(
                    tomogram_star_df,
                    f"{output_directory}/{tomo_name.removesuffix('.tomostar')}.star",
                )
                total_star_df = pd.concat(
                    [total_star_df, tomogram_star_df], ignore_index=True
                )
                # --- Write out .cmm files
                if cmm == True:
                    utils.cmm_write(
                        data=tomogram_star_df,
                        tomogram_name=tomo_name,
    ...
    particle_overview = dict(
        total_star_df["rlnMicrographName"].value_counts()
    )  # counts how many particles retrieved in each micrograph
    print("\n")
...
def decompress(input_dir=None, input_job=None, output_dir=None, non_interactive=False):
    ...
    if ask == "y":
        available_to_choose = [
            f"TS_{re.findall(r'\d+', file.split('/')[-1])}" for file in files
        ]
        print(f"Available tomograms to choose:\n{available_to_choose}")
        choices = re.findall(
            r"\d+",
            input(
                "Please list the tomograms you want to decompress. You only need to provide the number ID (i.e., TS_XXYY)/\n\nChoices:"
            ),
        )
...
def convert(input_dir, output_dir=None, data_type=None, non_interactive=False):
    ...
    if os.path.isfile(input_dir):
        ...
        file_list = utils.choose_tomograms(
            input_dir, caller="convert", non_interactive=non_interactive
        )
        file_list = [
            file for file in file_list if file.endswith(".mrc")
        ]  # this assumes that the tomograms are in mrc format - we can change this to be more flexible if needed
